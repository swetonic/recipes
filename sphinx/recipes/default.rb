...
cron_interval = 15 #If this is not set your data will NOT be indexed
...
if utility_name
  ...
  if ['solo', 'app', 'app_master'].include?(node[:instance_role])
    run_for_app(appname) do |app_name, data|
      ...
      execute "sphinx install 2.0.4 from src" do

        cwd "/data/#{app_name}/sphinx-2.0.4-release"
        command "./configure"

      end

      execute "sphinx install make" do
        cwd "/data/#{app_name}/sphinx-2.0.4-release"
        command "make"
      end

      execute "sphinx install make install" do
